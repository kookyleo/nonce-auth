...
use nonce_auth::{ConfigPreset, CredentialBuilder, CredentialVerifier, NonceConfig};
use std::sync::Arc;
use std::time::Duration;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Nonce Authentication with Optimized Configuration ===\n");

    // 1. Create storage backend for demonstration
    let memory_storage: Arc<dyn NonceStorage> = Arc::new(MemoryStorage::new());

    // 2. Configuration examples
    println!("=== Configuration Examples ===\n");

    let configs = vec![
        ("Production", NonceConfig::from(ConfigPreset::Production)),
        ("Development", NonceConfig::from(ConfigPreset::Development)),
        (
            "High-Security",
            NonceConfig::from(ConfigPreset::HighSecurity),
        ),
    ];
    ...
    let secret = b"production_secret_key_12345";
    ...
    for i in 0..100 {
        ...
        // Verify the credential with context and configuration
        match CredentialVerifier::new(Arc::clone(&memory_storage))
            .with_secret(secret)
            .with_context(Some("api_v1"))
            .with_storage_ttl(config.storage_ttl)
    ...
    let stats = memory_storage.get_stats().await?;
    println!("Storage Backend: {}", stats.backend_info);
    println!("Total Records: {}", stats.total_records);
    println!();

    // 6. Context isolation demonstration
    ...
    println!("Storage TTL: {:?}", config.storage_ttl);
    println!("Time Window: {:?}", config.time_window);
    println!();
    ...
    println!("4. Use ConfigPreset for common configuration patterns");
    println!("5. Custom configurations can be validated before use");
